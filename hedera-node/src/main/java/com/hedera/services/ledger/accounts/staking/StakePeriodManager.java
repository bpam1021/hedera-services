...
public class StakePeriodManager {
	...
	public long currentStakePeriod() {
		...
		if (prevConsensusSecs != currentConsensusSecs) {
			...
			currentStakePeriod = currentConsensusSecs /
					(properties.getLongProperty("staking.periodMins") * 60);
		}
	...
	public boolean isRewardable(final long stakePeriodStart) {
		...
		return stakePeriodStart > -1 && stakePeriodStart <= firstNonRewardableStakePeriod(); // need to change back  < = only for testing
	}
